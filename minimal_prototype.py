...
parser.add_argument('--missing_rates', type=float, nargs='+', default=[0.1, 0.3, 0.5, 0.7, 0.9],
                   help='要测试的缺失率列表 (0.1-0.9)')
parser.add_argument('--max_missing_rate', type=float, default=0.9,
                   help='动态训练的最大缺失率')
parser.add_argument('--epochs', type=int, default=100, help='训练轮数')
parser.add_argument('--data_path', type=str, default='./data/XJTU data/2C_battery-1.csv',
                   help='数据集路径')
...
def main():
    ...
    print(f"初始容量: {initial_capacity:.2f}, SOH范围: [{y_soh.min():.4f}, {y_soh.max():.4f}]")
    ...
    # 拆分数据集
    X_train, X_tmp, y_train, y_tmp = train_test_split(X_scaled, y_soh, test_size=0.3, random_state=args.seed)
    X_val, X_test, y_val, y_test = train_test_split(X_tmp, y_tmp, test_size=0.5, random_state=args.seed)
    
    print(f"数据集已拆分 - 训练集: {X_train.shape[0]}, 验证集: {X_val.shape[0]}, 测试集: {X_test.shape[0]}")
    ...
    full_train_dataset = BatteryDatasetFixedMissing(
        ...
        include_missing_indicators=True, missing_mask=full_train_mask
    )
    full_val_dataset = BatteryDatasetFixedMissing(
        X_val, y_val, missing_rate=0.0, 
        include_missing_indicators=True, missing_mask=full_val_mask
    )
    full_test_dataset = BatteryDatasetFixedMissing(
        X_test, y_test, missing_rate=0.0, 
        include_missing_indicators=True, missing_mask=full_test_mask
    )
    ...
    # 完整数据基线使用32个输入（16原始特征+16指示器，虽然指示器全为0）
    ...
    for i, missing_rate in enumerate(args.missing_rates):
        ...
        # 动态缺失率训练
        dyn_train_dataset = BatteryDatasetDynamicMissing(
            X_train, y_train, 
        ...
        train_loader = DataLoader(dyn_train_dataset, batch_size=args.batch_size, shuffle=True)
