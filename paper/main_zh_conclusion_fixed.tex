...
不同架构对MIM的受益程度存在显著差异。表\ref{tab:improvement_summary}显示，1D-CNN在MR=0.1--0.9全区间均保持较高改进率（45\%--57\%），在MR=0.9时MAE降至0.018（表\ref{tab:full_results}）。一个可能的原因是，1D-CNN的局部感受野结构有利于利用沿特征维度的局部模式，而$M$掩码帮助卷积核聚焦于真实观测较多的局部区域，降低插补噪声对卷积响应的干扰。LSTM和GRU在MR$\geq$0.5时改进率随缺失率上升（MR=0.9时分别为38\%和31\%），其时序门控机制可能在时间维度上平衡了有缺失和无缺失时间片的信息，缺失指示器帮助记忆单元区分"可信"与"不可信"输入。MLP在MR$\leq$0.6时改进明显（平均约46\%），但在MR=0.9时改进幅度降至27.9\%，这可能与其缺乏显式时序或局部结构建模能力有关——在极端高缺失率下，尽管MIM仍提供缺失位置信息，但全连接结构难以有效利用这一先验来重建特征关联。需要强调的是，上述机制解释仅为基于实验现象的合理推测，更严谨的理论解释仍有待未来工作通过消融实验和可解释性分析来验证。
...
\midrule
0.3$<$MR$\leq$0.6 & MLP-MIM / 1D-CNN-MIM & \textbf{复杂度：}低--中（参数量约$2.1\times10^4$--$3.1\times10^4$）；\textbf{适用硬件：}边缘计算节点、车载处理器；\textbf{场景：}中等数据缺失率的车队监测或储能站，MR=0.5时改进率46\%--55\%（表\ref{tab:improvement_summary}） \\
\midrule
...
在高缺失率区间（MR$\geq$0.7），若不使用MIM，误差和不稳定性均显著升高（Baseline在MR=0.9时MAE达0.028--0.036）。此时推荐1D-CNN-MIM或LSTM-MIM：前者在MR=0.9时MAE仅0.018，改进率51.0\%；后者MAE约0.018--0.019，改进率38.0\%--42.1\%（表\ref{tab:full_results}）。这两种架构适合部署于车队云端或储能站服务器端，能够在传感器故障高发条件下维持可用的SOH预测精度。

关于推理复杂度与实时性：基于第\ref{sec:methodology}章给出的参数量估计，MLP-MIM的参数规模约为$\mathcal{O}(10^4)$量级，前向推理仅需矩阵乘法与激活运算，适合在资源受限的嵌入式BMS上实现实时SOH预测。1D-CNN-MIM和LSTM-MIM参数量略高（约$2\times10^4$--$4\times10^4$），且涉及卷积或时序递归计算，但在车队云端或储能站服务器端推理时，其计算开销仍在可接受范围内。实际部署时，可根据目标硬件的算力和存储预算，在表\ref{tab:bms_selection}推荐的架构组合中选择最匹配的模型配置。

\subsection{研究边界与未来扩展方向}

本研究虽然在多模型、多缺失率条件下系统评估了MIM方法，但仍存在若干边界与局限。以下从三个维度分析这些局限及其对结果解释的影响，并给出针对性的未来工作方向。

\textbf{局限1：缺失机制。}当前实验仅考虑完全随机缺失（MCAR），未在更贴近实际的MAR（随机缺失）或MNAR（非随机缺失）场景下系统评估。在真实车队和储能数据中，缺失往往受工况、老化、运维策略的共同影响——例如，老化程度较高的电池可能更容易出现传感器故障，导致缺失与SOH本身相关。本文的MCAR假设可能低估或错估MIM在真实场景中的收益：若缺失与SOH存在相关性（MAR/MNAR），缺失模式$M$可能携带更多与目标变量相关的信息，MIM的潜在价值可能更高；反之，若缺失机制与现有特征存在复杂交互，简单拼接$[\hat{X}|M]$的策略可能需要调整。
...
随着电动汽车车队和电网级储能系统规模持续扩大，在缺失数据条件下保持SOH预测稳健性的需求愈发重要。本文仅是围绕"缺失即信号"范式的第一步系统实证，未来还需在真实BMS数据、物理模型融合以及调度/运维联动决策等层面进一步推进，才能真正支撑能源系统的安全与高效运行。

%==============================================================================

\section{结论与展望}\label{sec:conclusion}

本文围绕存在数据缺失条件下的电池SOH预测问题展开研究，采用"缺失即信号"的缺失指示器（MIM）范式。基于XJTU公开锂电池数据集，本文构建了循环级样本单元，设计了MR=0.1--0.9九档缺失率的MCAR缺失模拟，并对MLP、LSTM、GRU、1D-CNN四种典型深度学习架构进行了多缺失率、多随机种子的系统评估。实验结果表明，在中高缺失率（MR$\geq$0.4）条件下，MIM相比仅采用插补的Baseline在MAE、RMSE、R$^2$等指标上均取得显著性能提升，1D-CNN等架构的收益尤为明显，且经Wilcoxon符号秩检验等统计方法验证。本文工作为电池SOH预测中显式编码缺失模式的研究提供了系统性实证证据和可复现的评估框架。
...
未来研究仍有若干方向值得探索。本研究主要在MCAR缺失设定下完成，未来有必要在更接近实际BMS场景的MAR/MNAR机制下（缺失与工况、老化、运维策略相关）开展系统评估与对比。同时，在NASA、CALCE、Oxford等多源公开数据集，以及RUL预测、异常检测、多任务学习等更丰富任务上验证"缺失即信号"范式的通用性和边界，是重要后续方向。此外，将MIM与Transformer、图神经网络以及物理模型融合方法结合，并系统分析其在实际BMS部署中的算力与功耗代价，也具有研究价值。长远来看，将显式缺失建模与车队调度优化、电网级储能参与电力市场等决策系统深度耦合，是推动缺失鲁棒型SOH预测发挥系统级价值的关键方向。
