...
为系统评估缺失指示器方法在电池SOH预测中的作用，本文围绕三个问题展开：一是，在不同缺失率（MR）下，MIM相对仅插补方案能够带来多大性能与稳定性收益；二是，在MLP、LSTM、GRU与1D-CNN等典型深度架构中，MIM的收益是否存在显著差异；三是，在极端高缺失率条件下，哪些"模型 + MIM"组合仍能维持可接受的SOH预测精度与鲁棒性。
...
本文其余部分结构安排如下：第2章回顾锂电池SOH预测方法及缺失数据处理与MIM的相关研究；第3章给出本文的MIM输入策略与模型框架，包括样本构建、缺失模拟及多缺失率联合训练策略；第4章在XJTU数据集上呈现实验设置与多缺失率条件下的对比结果；第5章围绕MIM的作用机理与工程应用场景展开讨论，并给出面向BMS的模型选型建议；第6章总结全文工作及其对能源系统的潜在影响，并展望未来研究方向。
...
传统的缺失数据处理方法主要可以归纳为删除法、单一值插补法和基于模型的方法三大类。删除法在缺失率（MR）较低时简单有效，但在缺失率较高时会损失大量信息，且可能引入选择偏差。单一值插补方法（如均值插补、回归插补等）虽然保留了样本量，但会低估方差并扭曲变量间的关系。基于模型的方法如期望最大化（EM）算法、多重插补（Multiple Imputation）等通过建立概率模型处理缺失，理论上更为严谨，但计算复杂度高，通常需要对数据分布或缺失机制作出一定假设。
...
与现有文献相比，本研究的定位和主要区别体现在以下三个方面。首先，在研究视角上，现有工作多关注于提升SOH预测精度本身，而较少关注数据缺失这一实际工程中的普遍问题；本研究则将缺失数据处理作为核心研究内容，探索如何在存在缺失的条件下保持预测性能。其次，在方法选择上，现有缺失数据研究多聚焦于插补方法的改进，试图更准确地估计并填补缺失值；本研究则采用不同的思路，通过MIM方法将缺失信息显式编码，让模型自主学习如何利用这一信号。最后，在实验设计上，现有研究通常只在单一或少数缺失率下验证方法，而本研究覆盖了从0.1到0.9的九档缺失率（MR），能够更全面地评估方法在不同缺失程度下的表现。

综上所述，本文在电池SOH预测领域系统比较MIM方法在多种典型神经网络架构（MLP、LSTM、GRU、1D-CNN）和多种缺失率条件下的综合表现。通过在统一数据集和受控参数规模下开展大规模可重复实验，为缺失数据处理策略的选择提供了实证依据和工程指导。对不同缺失率和模型架构给出量化对比，对于电动汽车车队和电网级储能电站在BMS算法选型与升级决策方面具有直接的参考价值。
...
为系统评估不同缺失率（MR）条件下的模型性能，本研究采用完全随机缺失（MCAR）机制，在特征层面注入九档缺失率$\{0.1, 0.2, \ldots, 0.9\}$。MCAR机制下缺失位置的选择与特征值、SOH标签等均无关，便于控制实验条件，构成评估方法缺失鲁棒性的基准场景。
...
MIM是本研究的核心方法。对于每个原始特征，构造对应的二值缺失指示器变量：
\begin{equation}
    m_{ij} = 
    \begin{cases}
...
针对MIM模型，本研究提出多缺失率联合训练策略：将完整训练集在缺失率（MR）$\{0.0, 0.1, \ldots, 0.9\}$下分别生成10个带缺失副本，经MIM构造后合并为扩大10倍的增强训练集$\mathcal{D}_{\text{augmented}}$。在该增强集上训练统一的MIM模型，使其在训练阶段接触多种典型缺失模式，提升对未知缺失条件的鲁棒性。验证集固定采用MR=0.5，用于评估泛化表现。Baseline模型仅在完整数据（MR=0.0）上训练，因其无缺失指示器时训练数据中的缺失只会带来噪声。
...
本节围绕研究问题RQ1，评估在MR = 0.1--0.9不同缺失率（MR）下，MIM相对于仅插补Baseline的整体性能趋势。图\ref{fig:mae_comparison}(a)展示了四种模型在不同MR下的MAE曲线（含95\%置信区间），图\ref{fig:mae_comparison}(b)以热力图形式给出了九档MR下的MAE分布。
...
基于第\ref{sec:results}节的实验结果，本节对MIM的作用机制进行审慎分析。在第\ref{sec:results}节中，实验观察到以下一致现象：在MR$\geq$0.4的中高缺失率区间，四种深度学习架构（MLP、LSTM、GRU、1D-CNN）在MIM输入策略下的MAE均显著低于对应的Baseline版本，改进率约在26\%--57\%区间（表\ref{tab:improvement_summary}和图\ref{fig:mae_curves}）。在MR=0.9的极端缺失条件下，Baseline模型的MAE普遍超过0.028，而MIM组合（MLP-MIM、LSTM-MIM、GRU-MIM、1D-CNN-MIM）仍能维持在0.018--0.023的较低水平（表\ref{tab:full_results}）。Wilcoxon符号秩检验在关键缺失率（MR=0.3、0.5、0.7、0.9）下得到的$p$值均显著小于0.05，绝大多数小于0.001（表\ref{tab:significance}），表明MIM带来的改进具有统计显著性。

上述现象与MIM将缺失模式显式编码为输入的设计是一致的。传统Baseline方法仅接收插补后的特征矩阵$\hat{X}$，无法区分哪些元素是真实观测、哪些是插补估计。相比之下，MIM通过二值缺失指示矩阵$M$明确标记了每个特征位置的缺失状态，相当于为网络提供了一个"可信度权重（reliability weighting）"信号。在训练过程中，神经网络可以学习到对"观测值+缺失标签"的联合依赖关系，自动降低对高缺失、低置信度特征的依赖强度。即便在MCAR假设下，显式区分观测与插补也有助于限制插补误差的传播；而在更接近MAR或MNAR的实际场景中，缺失模式本身还可能携带与SOH相关的间接信息（如某些传感器故障与电池老化状态的关联性）。

不同架构对MIM的受益程度存在显著差异。表\ref{tab:improvement_summary}显示，1D-CNN在MR=0.1--0.9全区间均保持较高改进率（45\%--57\%），在MR=0.9时MAE降至0.018（表\ref{tab:full_results}）。一个可能的原因是，1D-CNN的局部感受野结构有利于利用沿特征维度的局部模式，而$M$掩码帮助卷积核聚焦于真实观测较多的局部区域，降低插补噪声对卷积响应的干扰。LSTM和GRU在MR$\geq$0.5时改进率随缺失率上升（MR=0.9时分别为38\%和31\%），其时序门控机制可能在时间维度上平衡了有缺失和无缺失时间片的信息，缺失指示器帮助记忆单元区分"可信"与"不可信"输入。MLP在MR$\leq$0.6时改进明显（平均约46\%），但在MR=0.9时改进幅度降至27.9\%，这可能与其缺乏显式时序或局部结构建模能力有关——在极端高缺失率下，尽管MIM仍提供缺失位置信息，但全连接结构难以有效利用这一先验来重建特征关联。需要指出的是，上述机制解释仅为基于实验现象的合理推测，更严谨的理论解释仍有待未来工作通过消融实验和可解释性分析来验证。

\subsection{面向 BMS 的模型与配置选型建议}

基于第\ref{sec:results}节的实验结果，本节将工程建议重构为表格形式（表\ref{tab:bms_selection}），便于BMS工程师快速查阅和实施。
...
\toprule
\textbf{缺失率区间} & \textbf{推荐架构组合} & \textbf{工程侧说明} \\
\midrule
MR$\leq$0.3 & MLP-Baseline / MLP-MIM & \textbf{复杂度：}低（参数量约$3.1\times10^4$）；\textbf{适用硬件：}嵌入式BMS芯片；\textbf{场景：}数据质量较好的单车或小规模储能单元，资源极度受限时可采用Baseline，否则推荐MLP-MIM作为默认选择 \\
\midrule
0.3$<$MR$\leq$0.6 & MLP-MIM / 1D-CNN-MIM & \textbf{复杂度：}低--中（参数量约$2.1\times10^4$--$3.1\times10^4$）；\textbf{适用硬件：}边缘计算节点、车载处理器；\textbf{场景：}中等缺失率（MR）的车队监测或储能站，MR=0.5时改进率46\%--55\%（表\ref{tab:improvement_summary}） \\
\midrule
MR$\geq$0.7 & 1D-CNN-MIM / LSTM-MIM & \textbf{复杂度：}中（参数量约$2.1\times10^4$--$3.5\times10^4$）；\textbf{适用硬件：}车队云端/储能站服务器；\textbf{场景：}高缺失率或传感器故障频发的工况，MR=0.9时MIM版本MAE维持在0.018--0.019（表\ref{tab:full_results}） \\
\bottomrule
\end{tabular}
\end{table}

在低缺失率区间（MR$\leq$0.3），Baseline模型已能取得较好的预测性能（MR=0.1时MAE约0.027，表\ref{tab:full_results}）。在此区间，引入MIM虽可进一步降低误差（降至约0.015），但相对收益有限。因此，在计算资源极为受限的嵌入式BMS芯片场景，可优先考虑MLP-Baseline；若对精度有一定冗余要求，则推荐MLP-MIM作为默认配置。

在中等缺失率区间（0.3$<$MR$\leq$0.6），缺失开始显著影响Baseline性能，而MIM的收益变得明显。以MR=0.5为例，MLP-MIM将MAE从0.030降至0.016，1D-CNN-MIM从0.034降至0.015（表\ref{tab:full_results}），改进率分别达到46.8\%和55.3\%（表\ref{tab:improvement_summary}）。两者分别代表了"轻量级高效方案"和"性能优先方案"，适合作为车队级监测或储能站边缘节点的通用部署选择。

在高缺失率区间（MR$\geq$0.7），若不使用MIM，误差和不稳定性均显著升高（Baseline在MR=0.9时MAE达0.028--0.036）。此时推荐1D-CNN-MIM或LSTM-MIM：前者在MR=0.9时MAE仅0.018，改进率51.0\%；后者MAE约0.018--0.019，改进率38.0\%--42.1\%（表\ref{tab:full_results}）。这两种架构适合部署于车队云端或电网级储能站服务器端，能够在传感器故障高发条件下维持可用的SOH预测精度。

关于推理复杂度与实时性：基于第\ref{sec:method}章给出的参数量估计，MLP-MIM的参数规模约为$\mathcal{O}(10^4)$量级，前向推理仅需矩阵乘法与激活运算，适合在资源受限的嵌入式BMS上实现实时SOH预测。1D-CNN-MIM和LSTM-MIM参数量略高（约$2\times10^4$--$4\times10^4$），且涉及卷积或时序递归计算，但在车队云端或电网级储能站服务器端推理时，其计算开销仍在可接受范围内。实际部署时，可根据目标硬件的算力和存储预算，在表\ref{tab:bms_selection}推荐的架构组合中选择最匹配的模型配置。

\subsection{研究边界与未来扩展方向}

本研究虽然在多模型、多缺失率条件下系统评估了MIM方法，但仍存在若干边界与局限。以下从三个维度分析这些局限及其对结果解释的影响，并给出针对性的未来工作方向。

\textbf{局限1：缺失机制。}当前实验仅考虑完全随机缺失（MCAR），未在更贴近实际的MAR（随机缺失）或MNAR（非随机缺失）场景下系统评估。在真实车队和电网级储能数据中，缺失往往受工况、老化、运维策略的共同影响——例如，老化程度较高的电池可能更容易出现传感器故障，导致缺失与SOH本身相关。本文的MCAR假设可能低估或错估MIM在真实场景中的收益：若缺失与SOH存在相关性（MAR/MNAR），缺失模式$M$可能携带更多与目标变量相关的信息，MIM的潜在价值可能更高；反之，若缺失机制与现有特征存在复杂交互，简单拼接$[\hat{X}|M]$的策略可能需要调整。

\textit{未来方向：}在实际BMS采集数据上标注缺失原因（如传感器故障代码、通信日志），构建更真实的MAR/MNAR缺失机制假设；对比MIM与显式建模缺失机制的方法（如基于掩码的时序模型、生成式缺失建模）在这些场景下的表现，系统刻画MIM的适用边界。

\textbf{局限2：数据集与任务范围。}本研究仅使用XJTU单一数据集和单一SOH回归任务进行验证。结果的可迁移性和普适性仍有限，尚不能直接外推到NASA、CALCE、Oxford等其他公开数据集，或剩余使用寿命（RUL）预测、故障诊断等其他电池健康管理任务。不同数据集的采集条件、电池化学体系、工况分布存在差异，MIM策略在这些场景下的有效性需要独立验证。

\textit{未来方向：}在多数据集（NASA、CALCE、Oxford等）、多任务（SOH/RUL/异常检测）上复现实验，以验证"缺失即信号"范式的通用性；针对实际BMS中可能存在的数据分布偏移问题，研究MIM与领域自适应（Domain Adaptation）方法的结合。

\textbf{局限3：插补策略与模型家族。}本文仅采用均值插补作为统一基线，模型也仅涵盖MLP、LSTM、GRU、1D-CNN四类经典架构。尚未系统比较"插补方法的改进空间"与"是否采用MIM"的相对重要性，也未评估更复杂的注意力架构（如Transformer）的收益与代价。若采用更复杂的插补方法（如KNN、迭代插补、生成模型），Baseline性能可能提升，MIM的相对改进幅度可能发生变化。

\textit{未来方向：}引入多种插补方法（KNN、回归插补、矩阵分解、生成模型等）并与MIM组合对比，验证"MIM+简单插补"是否足以媲美复杂插补策略；探索MIM与Transformer、图神经网络（GNN）等新兴架构的结合，同时量化其在BMS场景中的算力和功耗开销，评估工程可行性。

随着电动汽车车队和电网级储能系统规模持续扩大，在缺失数据条件下保持SOH预测稳健性的需求愈发重要。本文围绕"缺失即信号"范式开展了系统实证，未来还需在真实BMS数据、物理模型融合以及调度/运维联动决策等层面进一步推进，才能真正支撑能源系统的安全与高效运行。

%==============================================================================

\section{结论与展望}\label{sec:conclusion}

本文围绕存在数据缺失条件下的电池SOH预测问题展开研究，采用"缺失即信号"的MIM范式。基于XJTU公开锂电池数据集，本文构建了循环级样本单元，设计了MR=0.1--0.9九档缺失率（MR）的MCAR缺失模拟，并对MLP、LSTM、GRU、1D-CNN四种典型深度学习架构进行了多缺失率、多随机种子的系统评估。实验结果表明，在中高缺失率（MR$\geq$0.4）条件下，MIM相比仅采用插补的Baseline在MAE、RMSE、R$^2$等指标上均取得显著性能提升，1D-CNN等架构的收益尤为明显，且经Wilcoxon符号秩检验等统计方法验证。本文工作为电池SOH预测中显式编码缺失模式的研究提供了系统性实证证据和可复现的评估框架。

在电动汽车车队和电网级储能系统等实际应用场景中，BMS需要在传感器故障、通信丢包等不完美数据条件下持续估计SOH，本文的结果为此类场景下的模型与输入策略选型提供了实证依据。在车队应用中，采用MIM的深度模型（特别是1D-CNN和LSTM架构）可在中高缺失率下保持更稳健的SOH估计，有助于降低健康状态误判风险，优化充电策略和维护调度决策。在电网级储能站，长期运行、传感老化和复杂工况导致数据缺失更为常见，本研究显示合理选取"模型+MIM"组合可在高缺失条件下显著抑制预测误差和不稳定性，为调频、削峰填谷等业务提供更可靠的健康信息支撑。上述发现对BMS软件算法设计和硬件算力配置具有直接参考价值，有望提升电池资产利用效率和运行安全性。

未来研究仍有若干方向值得探索。本研究主要在MCAR缺失设定下完成，未来有必要在更接近实际BMS场景的MAR/MNAR机制下（缺失与工况、老化、运维策略相关）开展系统评估与对比。同时，在NASA、CALCE、Oxford等多源公开数据集，以及RUL预测、异常检测、多任务学习等更丰富任务上验证"缺失即信号"范式的通用性和边界，是重要后续方向。此外，将MIM与Transformer、图神经网络以及物理模型融合方法结合，并系统分析其在实际BMS部署中的算力与功耗代价，也具有研究价值。将显式缺失建模与车队调度优化、电网级储能参与电力市场等决策系统深度耦合，是推动缺失鲁棒型SOH预测发挥系统级价值的关键方向。
