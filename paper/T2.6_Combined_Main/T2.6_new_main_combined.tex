...
Figure~\ref{fig:problem_scenario} illustrates the BMS monitoring chain and the corresponding cycle-level feature matrix, where multi-source failures lead to random missing patterns in the feature matrix. This schematic demonstrates how sensor failures, communication interruptions, and sampling strategy optimizations contribute to data incompleteness in real-world battery management systems.
...
Regarding missing data problems in BMS monitoring data, existing research roughly adopts three categories of treatment strategies: deletion methods, imputation methods, and model-based methods. Deletion methods avoid processing missing values by directly discarding samples or features containing missing values, but in high missing rate scenarios, this causes severe information loss and selection bias. Imputation methods estimate and fill missing values through statistics or prediction models, and are currently the most widely used in industry. However, such methods struggle to characterize the uncertainty introduced by missing data and cannot explicitly encode the potentially useful information of ``which features are missing where.'' Model-based methods are theoretically more rigorous, but often require complex distribution assumptions, with high computational costs and implementation difficulties, making direct deployment difficult in resource-constrained BMS environments.

The above traditional methods generally follow the ``repair data first, then predict'' processing paradigm, treating missing data as noise to be eliminated rather than potential health state indicator signals. In contrast, the Missing Indicator Matrix (MIM) method provides an alternative approach by explicitly encoding missing patterns as additional binary features, feeding them into the model together with original features\cite{jones1996indicator,van2023missing}. This ``missing as signal'' paradigm has been successfully applied in medical data analysis, recommendation systems, and other domains\cite{sisk2023imputation,ehrig2025imputation,ipsen2022deal,che2018recurrent}, showing significant performance improvements when missing patterns are informative.

%------------------------------------------------------------------------------
\subsection{Research Gap and Contributions}\label{sec:contributions}

Despite the availability of rich datasets and advanced deep learning techniques, most existing SOH prediction research assumes that input data is essentially complete, with less systematic consideration of data missing problems\cite{shao2025soh,deng2025novel,chen2024batteryMissing}. This study identifies three key gaps in the current literature:

\begin{enumerate}
    \item \textbf{Gap in missing data consideration:} Existing SOH estimation work predominantly assumes data completeness, lacking systematic investigation into how various missing rates affect prediction performance in real-world BMS applications.
    
    \item \textbf{Gap in unified evaluation framework:} There is a lack of comprehensive evaluation frameworks that systematically assess MIM effectiveness across multiple missing rates and diverse deep learning architectures under strictly controlled experimental conditions.
    
    \item \textbf{Gap in engineering guidance:} Current literature provides limited actionable guidance for BMS practitioners on selecting appropriate model architectures and MIM configurations based on specific missing rate intervals and hardware constraints.
\end{enumerate}

To address these gaps, this paper proposes the ``missing as signal'' modeling paradigm and systematically evaluates the MIM method for battery SOH prediction. The core contributions of this work are summarized as follows:

\begin{itemize}
    \item \textbf{Paradigm innovation:} Systematically proposing and validating the ``missing as signal'' paradigm in battery SOH prediction, shifting from the traditional ``impute first, then predict'' workflow to an end-to-end joint learning approach that treats missing patterns as first-order input features.
    
    \item \textbf{Experimental framework:} Constructing a unified evaluation benchmark covering nine levels of missing rates, four representative deep learning architectures, and rigorous statistical validation through 100 random seeds, providing comprehensive insights into MIM effectiveness across diverse scenarios.
    
    \item \textbf{Training strategy:} Proposing a multi-missing-rate joint training strategy that enables a single MIM model to maintain robust performance when facing unknown missing patterns, enhancing practical deployability in complex operating conditions.
    
    \item \textbf{Engineering guidelines:} Providing quantitative model selection recommendations mapping missing rate intervals to optimal architecture-MIM combinations, offering actionable guidance for BMS design in electric vehicles and energy storage systems.
\end{itemize}

%------------------------------------------------------------------------------
\subsection{Paper Organization}\label{sec:organization}

The remainder of this paper is organized as follows. Section~\ref{sec:method} presents the proposed methodology, including problem formulation, MIM input construction, multi-missing-rate joint training strategy, and deep learning architectures. Section~\ref{sec:dataset} describes the XJTU battery dataset, feature engineering, missing simulation settings, and experimental setup. Section~\ref{sec:results} presents the experimental results, performance analysis, and statistical significance tests. Section~\ref{sec:results} discusses the MIM mechanism, architecture-wise analysis, and BMS-oriented configuration recommendations. Finally, Section~\ref{sec:conclusion} summarizes the findings and outlines future research directions.
...
Traditional approaches to missing data in SOH prediction follow a two-stage ``impute first, then predict'' paradigm, where missing values are filled before model training, and the missing indicator information is discarded. This study proposes an alternative ``missing as signal'' paradigm that explicitly encodes missing patterns as informative features.
...
\textbf{Baseline Strategy.} The baseline approach uses only the imputed feature vector as input:
\begin{equation}
...
\textbf{MIM Strategy.} The Missing Indicator Matrix (MIM) approach augments the imputed features with the binary missing indicator vector:
\begin{equation}
...
\textbf{Baseline Training.} Baseline models are trained exclusively on complete data (MR = 0.0). Training on data with missing values without explicit missing indicators introduces noise that the model cannot distinguish from actual feature variation, potentially degrading performance.

\textbf{MIM Training.} MIM models are trained on the augmented dataset $\mathcal{D}_{\text{train}}^{\text{aug}}$ containing samples with varying missing rates. During each training iteration, mini-batches are sampled from this augmented set, exposing the model to diverse missing patterns. This enables the model to learn robust representations that generalize across different missing conditions.
...
The Multilayer Perceptron serves as the simplest baseline, learning direct mappings from input features to SOH without explicit sequential or spatial assumptions. The architecture consists of four fully-connected hidden layers with dimensions progressively reducing from input to output:
\begin{equation}
...
\textbf{Early Stopping.} Training is halted when validation loss ceases to improve for a specified number of consecutive epochs (patience). This prevents overfitting and ensures model generalization.

\textbf{Dropout Regularization.} Dropout is applied at specified rates after hidden layers (see Table~\ref{tab:model_config}), randomly deactivating neurons during training to prevent co-adaptation.

\textbf{Batch Normalization.} Not explicitly used; Z-score normalization is applied to input features as a preprocessing step (detailed in Section~\ref{sec:dataset}).
...
This section describes the experimental platform, data preprocessing pipeline, missing data simulation protocol, evaluation methodology, and implementation environment. All experiments are conducted on the publicly available XJTU battery aging dataset under strictly controlled conditions to ensure reproducibility and fair comparison.
...
\textbf{Battery Specifications and Test Conditions.} The dataset encompasses multiple lithium-ion battery chemistries, including Lithium Iron Phosphate (LFP) and Nickel Manganese Cobalt (NMC) oxide, which represent mainstream cathode materials in current electric vehicle and energy storage applications. The batteries were subjected to various charge-discharge protocols with different current rates and operating conditions, covering typical operational scenarios encountered in practical applications.

\textbf{Data Scope and Scale.} After rigorous data quality screening and preprocessing, the experimental corpus comprises approximately 9,000 valid cycle-level samples spanning the complete aging trajectory from initial state (State of Health $\approx$ 100\%) to end-of-life criteria (State of Health dropping to approximately 50\%). This extensive coverage ensures that the evaluation encompasses the full operational lifecycle of battery systems.
...
\textbf{Voltage Statistical Features (4 dimensions).} Mean voltage, standard deviation, skewness, and kurtosis are computed from the voltage curves during cycling. These statistical moments capture the overall morphology of voltage distribution, reflecting changes in battery internal resistance, polarization characteristics, and electrochemical stability over the aging process.

\textbf{Charging Process Features (4 dimensions).} Constant-current phase charging capacity and duration, voltage slope during charging, and voltage entropy are extracted to characterize the battery's charging acceptance capability and voltage signal complexity. These indicators are sensitive to lithium plating, electrolyte degradation, and electrode structural changes.

\textbf{Current Statistical Features (8 dimensions).} Mean current, standard deviation, skewness, and kurtosis, combined with constant-voltage phase charging capacity, duration, current slope, and current entropy, comprehensively characterize current fluctuation patterns and constant-voltage phase degradation behavior. These features provide complementary information to voltage-based indicators.
...
\textbf{Outlier Removal.} Cycles exhibiting physically anomalous SOH values (SOH $>$ 100\% indicating measurement errors, or SOH $<$ 50\% representing end-of-life regions with highly unstable characteristics) are excluded from the analysis to maintain data reliability.

\textbf{Noise Smoothing.} Continuous features such as voltage and current profiles are subjected to anomaly detection and smoothing procedures to mitigate sensor noise and transient measurement fluctuations while preserving the underlying aging trends.

\textbf{Sequence Alignment.} Cycle sequences are aligned and verified for continuity to ensure temporal consistency, eliminating discontinuities that could confound temporal modeling approaches.

\textbf{Feature Standardization.} All features undergo Z-score standardization prior to model input:
\begin{equation}
...
%------------------------------------------------------------------------------
\subsection{Missing Simulation and Imputation Settings}\label{sec:missing}
...
This imputation approach is selected for its computational simplicity, implementation stability, and widespread adoption in industrial practice. By using a consistent, straightforward imputation method as the foundation, the subsequent performance gains attributable to the MIM strategy can be clearly isolated and quantified.
...
\begin{itemize}
    \item \textbf{Training Set (60\%):} Approximately 18 batteries, comprising $\approx$6,000 cycle samples for model parameter learning.
    \item \textbf{Validation Set (20\%):} Approximately 6 batteries, comprising $\approx$2,000 cycle samples for hyperparameter tuning and early stopping.
    \item \textbf{Test Set (20\%):} Approximately 6 batteries, comprising $\approx$2,000 cycle samples reserved exclusively for final performance evaluation.
\end{itemize}

\textbf{Multi-Missing-Rate Evaluation Protocol.} For each trained model, evaluation is conducted across all nine missing rate levels on the held-out test set. The validation set employs a fixed missing rate of MR = 0.5 for monitoring generalization during training, providing a balanced assessment condition.

\subsubsection{Evaluation Metrics}

Model performance is quantified using three standard regression evaluation metrics:

\textbf{Mean Absolute Error (MAE).} MAE measures the average magnitude of prediction errors without considering their direction:
\begin{equation}
...
\textbf{Root Mean Squared Error (RMSE).} RMSE penalizes larger errors more heavily through quadratic weighting:
\begin{equation}
...
Figure~\ref{fig:mae_comparison} presents the MAE performance comparison between Baseline and MIM strategies across nine missing rate levels (MR = 0.1--0.9) for all four deep learning architectures. The results reveal three consistent trends:

\textbf{Monotonic degradation with increasing missing rate.} All models exhibit increasing MAE as MR rises, reflecting the fundamental challenge of prediction under information scarcity. At MR = 0.1 (minor data gaps), both Baseline and MIM achieve relatively low errors; however, as MR approaches 0.9 (severe missing), Baseline models experience substantial performance degradation while MIM-enabled models maintain more stable accuracy.

\textbf{Universal MIM advantage across missing rates.} MIM consistently outperforms Baseline at every tested missing rate. At low missing rates (MR = 0.1), MIM provides modest improvements of approximately 8.6\%--45.2\% depending on architecture. At moderate missing rates (MR = 0.5), improvements become more pronounced, reaching 26\%--57\% error reduction. The 1D-CNN architecture with MIM achieves the highest improvement rate at MR = 0.5 (55.3\%), followed by MLP-MIM (46.8\%).

\textbf{Amplified benefits under high missing rates.} The performance gap between MIM and Baseline widens significantly at high missing rates (MR $\geq$ 0.7). While Baseline MAE generally exceeds 0.032 in this regime, MIM versions maintain MAE below 0.023. At the extreme case of MR = 0.9, Baseline errors typically exceed 0.029, whereas MIM-enabled models sustain errors below 0.020, demonstrating robustness under severe data incompleteness.
...
\textbf{1D-CNN: Consistent high performance.} The 1D-CNN architecture demonstrates the most robust performance across all missing rates when combined with MIM. Its local receptive field structure appears particularly well-suited to processing the extended input $[\hat{X} | M]$, allowing convolutional filters to simultaneously capture local feature patterns and their corresponding missing status. At extreme missing rates (MR = 0.9), 1D-CNN-MIM maintains MAE as low as 0.018, representing a 51\% improvement over Baseline.

\textbf{LSTM and GRU: Temporal gating benefits.} Recurrent architectures show improvement rates that increase with missing rate, suggesting their gating mechanisms effectively utilize missing indicators to modulate information flow. At MR = 0.9, both LSTM-MIM and GRU-MIM achieve MAE around 0.018--0.019, comparable to 1D-CNN-MIM. This indicates that temporal models can leverage missing pattern information to maintain prediction accuracy even when substantial historical data is unavailable.

\textbf{MLP: Effective but limited at extremes.} While MLP-MIM achieves substantial improvements at low-to-moderate missing rates (average 46\% at MR $\leq$ 0.6), its improvement drops to 27.9\% at MR = 0.9. This suggests that fully-connected architectures, lacking explicit spatial or temporal structure, struggle to fully exploit missing pattern information under extreme data scarcity.
...
Figure~\ref{fig:model_selection} provides a visual decision guide for BMS practitioners, mapping missing rate intervals to recommended model-MIM configurations based on the comprehensive experimental results.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{fig12_model_selection_v2.png}
    \caption{BMS-oriented model selection decision guide based on missing rate intervals and computational constraints.}
    \label{fig:model_selection}
\end{figure}

%------------------------------------------------------------------------------
\subsection{Limitations and Discussion}\label{sec:limitations}

While this study demonstrates the effectiveness of MIM for battery SOH prediction under missing data conditions, several limitations warrant consideration:

\textbf{Missing mechanism scope.} Current experiments adopt the MCAR assumption for controlled evaluation. Real-world BMS data may exhibit MAR (Missing at Random) or MNAR (Missing Not at Random) patterns where missingness correlates with battery aging state or operational conditions. Under such mechanisms, missing patterns may carry additional information about SOH, potentially enhancing MIM's value, or may interact with features in complex ways requiring more sophisticated modeling than simple concatenation.

\textbf{Dataset and task generality.} Results are validated on the XJTU dataset covering specific chemistries and operating conditions. Generalization to other battery chemistries, drive cycles, or environmental conditions requires additional validation. Extension to related tasks such as remaining useful life (RUL) prediction or fault diagnosis also remains to be explored.

\textbf{Methodological scope.} The study adopts mean imputation as the baseline and evaluates four classical deep learning architectures. Alternative imputation strategies (KNN, iterative methods, generative models) may alter Baseline performance and consequently MIM's relative improvements. Emerging architectures such as Transformers or Graph Neural Networks may offer additional capabilities for handling missing data.

These limitations define boundaries for interpreting current results and identify directions for future investigation, including validation under realistic missing mechanisms, multi-dataset evaluation, and exploration of advanced imputation and modeling techniques.
...
\begin{itemize}
    \item \textbf{MIM consistently outperforms traditional imputation-based baselines across the full spectrum of missing rates.} The performance advantage is particularly pronounced at moderate-to-high missing rates, where the explicit encoding of missing patterns enables models to maintain prediction accuracy despite substantial information loss.
    
    \item \textbf{Multi-missing-rate joint training enhances model robustness to unknown missing conditions.} By exposing models to diverse missing patterns during training, the proposed strategy enables single models to generalize effectively across varying degrees of data incompleteness, mitigating the instability associated with training solely on complete data.
    
    \item \textbf{Architecture-specific behaviors reveal differential suitability for missing data scenarios.} Convolutional and recurrent architectures demonstrate particular advantages in leveraging missing pattern information, maintaining low prediction errors even under extreme missing rates where simpler feedforward structures exhibit performance degradation.
    
    \item \textbf{MIM reduces model sensitivity to random initialization and data partitioning.} Cross-seed stability analysis indicates that incorporating missing indicators not only improves average prediction accuracy but also decreases result variance, enhancing reproducibility and reliability for practical deployment.
\end{itemize}
...
\textbf{Missing-rate-adaptive model selection.} For deployment scenarios with varying data quality characteristics, the relationship between observed missing rate and optimal model configuration can guide implementation decisions. Low-complexity architectures with MIM suffice for environments with minor data gaps, while more sophisticated models become warranted as missing data prevalence increases.

\textbf{Information preservation strategy.} Rather than treating missing data purely as a preprocessing problem to be eliminated through imputation, BMS designers should consider explicit missing pattern encoding as an integral component of the prediction pipeline. This paradigm shift from ``repair then predict'' to ``predict with awareness'' can substantially enhance system robustness under sensor degradation or communication failures.

\textbf{Computational resource allocation.} The trade-off between model complexity and prediction accuracy varies with missing rate severity. Resource-constrained edge deployments can prioritize lightweight configurations for low-to-moderate missing rates, while cloud-based fleet monitoring systems can leverage more sophisticated architectures to handle severe data incompleteness.

\textbf{Stability and reproducibility considerations.} The observed reduction in cross-seed variance with MIM suggests that explicit missing modeling can enhance system reliability across different vehicles, operational periods, and sensor configurations—a critical requirement for safety-critical battery health monitoring applications.

%------------------------------------------------------------------------------
\subsection{Limitations and Future Work}\label{sec:future}

While this study provides systematic evidence for the effectiveness of missing-aware SOH prediction, several limitations warrant acknowledgment and motivate future investigation:

\textbf{Missing mechanism scope.} The current evaluation adopts the Missing Completely at Random assumption for controlled experimental assessment. Real-world BMS data may exhibit more complex missing patterns where sensor failures correlate with battery aging state or operational conditions. Future work should validate MIM effectiveness under Missing at Random and Missing Not at Random mechanisms, potentially incorporating causal modeling of missingness into the prediction framework.

\textbf{Dataset and task generalization.} Results are validated on a single laboratory dataset with specific battery chemistries and cycling protocols. Extension to diverse battery technologies, drive cycle patterns, and environmental conditions is necessary to establish broad applicability. Additionally, exploration of MIM for related battery health management tasks—such as remaining useful life prediction and anomaly detection—remains to be investigated.

\textbf{Methodological extensions.} The current study adopts mean imputation as the baseline and evaluates established deep learning architectures. Future research could explore advanced imputation strategies (such as deep generative models or temporal interpolation networks) in conjunction with MIM, as well as emerging model families (including attention-based architectures and physics-informed neural networks) that may offer complementary capabilities for handling incomplete battery data.
