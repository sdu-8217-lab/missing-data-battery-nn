...
% Title information
\title{\textbf{Comparative Study of Missing Indicator-Based State of Health Prediction for Lithium-Ion Batteries: \\Deep Learning Models and Multi-Missing-Rate Experiments on XJTU Dataset}}
\author{Author Name$^{1}$ \quad Corresponding Author$^{2}$}
\date{$^{1}$Author Affiliation \\ $^{2}$Corresponding Author Affiliation \\ \texttt{email@example.com}}
...
%==============================================================================
\begin{abstract}
Accurate prediction of lithium-ion battery State of Health (SOH) is of critical importance for ensuring the safe and reliable operation of electric vehicles and energy storage systems. However, in practical engineering applications, battery monitoring data commonly suffers from varying degrees of missing data due to sensor failures, communication interruptions, data acquisition anomalies, and other factors. Traditional missing data treatment methods typically regard missing values as noise to be repaired, while overlooking the potentially useful signal contained in ``which information is missing'' itself, which particularly limits the performance ceiling of prediction models under high missing rate scenarios.

This study employs the publicly available XJTU lithium-ion battery dataset as the research subject, introducing the simple-to-implement Missing Indicator Matrix (MIM) method to systematically evaluate its impact on SOH prediction performance and robustness under various commonly used model architectures and different missing rate conditions. In the data preprocessing stage, we extract 16 cycle-level statistical features with cycles as sample units and construct cycle-level SOH labels; in the missing simulation stage, we inject nine levels of missing rates (MR) $\{0.1, 0.2, \ldots, 0.9\}$ randomly at the feature level and adopt mean imputation to fill missing values; in the feature enhancement stage, we construct binary missing indicators for each feature and use ``imputed values concatenated with indicators'' as MIM input. We select four representative deep learning models (MLP, LSTM, GRU, and 1D-CNN), conduct unified training and evaluation under the PyTorch 2.10.0 deep learning framework, and compare and analyze the differences in SOH prediction error and robustness between ``imputation-only'' and ``MIM input'' strategies under different missing rates.

Experimental results indicate that under missing rate $\geq 0.4$ conditions, introducing the MIM version significantly reduces SOH prediction error across all four neural network models (improvement rates of 26\%--57\%). Specifically, at MR=0.5, 1D-CNN-MIM reduces MAE from 0.034 to 0.015 (improvement rate of 55.3\%), while LSTM-MIM reduces it from 0.022 to 0.016 (improvement rate of 29.0\%). Under extreme conditions at MR=0.9, baseline models generally exhibit MAE exceeding 0.029, whereas MIM versions can still maintain values below 0.029, demonstrating significant missing data robustness. Further comparative analysis reveals that performance differences among different imputation methods are relatively limited, while whether to use missing indicators has a more critical impact on prediction performance. This study provides a concise yet effective technical solution for data missing problems in practical Battery Management Systems (BMS), holding important theoretical and practical value for improving the applicability of data-driven SOH prediction methods in real-world engineering environments.

\textbf{Keywords:} lithium-ion battery; State of Health (SOH) prediction; missing data; Missing Indicator Matrix (MIM); XJTU dataset; deep learning
\end{abstract}
...
The remainder of this paper is organized as follows: Section~2 reviews research on lithium battery SOH prediction methods and missing data treatment with MIM; Section~3 presents the MIM input strategy and model framework of this paper, including sample construction, missing simulation, and multi-missing-rate joint training strategy; Section~4 presents experimental settings and comparative results under multi-missing-rate conditions on the XJTU dataset; Section~5 discusses the mechanism of MIM and engineering application scenarios, and provides model selection recommendations for BMS; Section~6 summarizes the work and its potential impact on energy systems, and outlines future research directions.

%==============================================================================
\section{Related Work and Technical Background}\label{sec:related}

\subsection{Overview of Lithium Battery SOH Prediction Methods}

In electric vehicle fleets and grid-scale energy storage systems, accurate estimation of battery SOH is directly related to system safety, economy, and lifetime management. Battery SOH prediction, as a typical regression problem, has evolved from physics-based to data-driven modeling approaches\cite{liu2025deep}. Early physics-based methods described internal battery aging mechanisms through electrochemical impedance spectroscopy and equivalent circuit models, offering clear physical interpretability, but with numerous model parameters that are difficult to identify, and poor adaptability to different chemistries and operating conditions. With the development of machine learning technology, data-driven methods have gradually become mainstream\cite{liu2025deep}, which can be roughly divided into three categories based on model structural characteristics: feedforward networks (MLP), recurrent neural networks (LSTM/GRU), and one-dimensional convolutional neural networks (1D-CNN).

Multilayer Perceptron (MLP), as the most basic neural network structure, learns complex mapping relationships between features and SOH through multi-layer nonlinear transformations, featuring simple structure and controllable parameter count, making it suitable as a baseline method and for resource-constrained scenarios\cite{bairwa2025cycle}. Temporal models, represented by LSTM and GRU, can explicitly model temporal dependencies between adjacent cycles in terms of capacity and features, utilizing historical information to assist current prediction, demonstrating better stability in complex situations such as capacity regeneration and aging trend turning points. One-dimensional convolutional neural networks (1D-CNN) extract local feature patterns along the temporal dimension through convolution kernels, featuring high parallel computational efficiency and strong local pattern recognition capabilities, performing excellently under conditions with relatively limited training data\cite{zhang2024cnnsoh}. The above three architectures each have advantages in capturing temporal dependencies and cross-cycle features, and have been widely used for battery SOH prediction and remaining useful life estimation tasks on unified datasets\cite{bairwa2025cycle}.
...
\subsection{Missing Data Treatment and Missing Indicator Matrix (MIM)}

In practical data analysis tasks, data missing is a ubiquitous challenge. From a statistical perspective, missing mechanisms are typically classified into three categories: missing completely at random (MCAR), where missingness is unrelated to any observed or unobserved variables; missing at random (MAR), where missingness can be explained by observed variables; and missing not at random (MNAR), where missingness is related to the unobserved variable values themselves\cite{little2002statistical}. Under different missing mechanisms, appropriate treatment strategies also vary.
...
In summary, for battery SOH prediction tasks, existing research still has key gaps that urgently need systematic filling in areas such as unified evaluation frameworks for multiple missing rates, cross-architecture MIM effect comparisons, and publicly available comparative experiments with strictly controlled variables. Filling these gaps helps clarify the real benefits and applicable boundaries of MIM under missing data conditions on one hand, and provides quantitative basis for selecting BMS missing data treatment strategies in engineering practice on the other.

Compared with existing literature, the positioning and main distinctions of this research are reflected in three aspects. First, in terms of research perspective, existing work mostly focuses on improving SOH prediction accuracy itself, while paying less attention to data missing, a common problem in practical engineering; this research treats missing data treatment as core research content, exploring how to maintain prediction performance under missing conditions. Second, in terms of method selection, existing missing data research mostly focuses on improving imputation methods, attempting to more accurately estimate and fill missing values; this research adopts a different approach, explicitly encoding missing information through the MIM method and allowing the model to autonomously learn how to utilize this signal. Finally, in terms of experimental design, existing research usually validates methods only under single or few missing rates, while this research covers nine levels of missing rates (MR) from 0.1 to 0.9, enabling more comprehensive evaluation of method performance under different missing degrees.

In summary, this paper systematically compares the comprehensive performance of the MIM method across multiple typical neural network architectures (MLP, LSTM, GRU, 1D-CNN) and various missing rate conditions in the battery SOH prediction domain. By conducting large-scale reproducible experiments under unified datasets and controlled parameter scales, empirical evidence and engineering guidance are provided for missing data treatment strategy selection. The quantitative comparison for different missing rates and model architectures has direct reference value for BMS algorithm selection and upgrade decisions in electric vehicle fleets and grid-scale energy storage stations.
...
After data cleaning, the dataset retains approximately 9,000 valid cycle samples, covering the complete aging process from initial state to SOH dropping to approximately 50\%. This study constructs 16-dimensional cycle-level statistical features from three dimensions: voltage, current, and charging process (specific definitions and physical meanings are shown in Appendix A Table~\ref{tab:features}):
...
Model architecture and parameter configurations are summarized in Table~\ref{tab:model_config}. Due to MIM input dimension increasing from 16 to 32, parameter counts increase accordingly (see Appendix A for parameter count calculation examples).
...
\begin{tabular}{@{}lcl@{}}
\toprule
\textbf{Hyperparameter} & \textbf{Configuration} & \textbf{Description} \\
\midrule
Optimizer & Adam & Adaptive learning rate optimization \\
Learning rate & $10^{-3}$ & Initial learning rate \\
Batch size & 32 & Mini-batch gradient descent \\
Maximum epochs & 200 & Maximum training epochs before early stopping \\
Early stopping patience & 15 rounds & Stop if validation loss does not decrease \\
...
\toprule
\textbf{Subset} & \textbf{Proportion} & \textbf{\#Batteries} & \textbf{\#Cycles} & \textbf{Purpose} \\
\midrule
Training set & 60\% & $\approx$18 & $\approx$6,000 & Model parameter learning \\
Validation set & 20\% & $\approx$6 & $\approx$2,000 & Hyperparameter tuning, early stopping \\
Testing set & 20\% & $\approx$6 & $\approx$2,000 & Final performance evaluation \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{MAE curves varying with missing rate}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{MAE heatmap (model $\times$ missing rate)}
...
%==============================================================================
\appendix
\section{Appendix A: Supplementary Experimental Results and Technical Details}

\subsection{Cycle-Level Feature Extraction Details}

Table~\ref{tab:features} provides detailed definitions and physical meanings of the 16-dimensional cycle-level statistical features.

\begin{table}[H]
\centering
\caption{Cycle-Level Feature Extraction Details}
\label{tab:features}
\begin{tabular}{@{}llcp{6cm}@{}}
\toprule
\textbf{Category} & \textbf{Feature Name} & \textbf{Dimension} & \textbf{Physical Meaning} \\
\midrule
\multirow{4}{*}{Voltage Statistics} 
& voltage\_mean & 1 & Arithmetic mean of voltage sampling values within cycle, reflecting battery average operating voltage \\
& voltage\_std & 1 & Voltage standard deviation, characterizing voltage fluctuation degree \\
& voltage\_skewness & 1 & Voltage skewness, characterizing voltage distribution asymmetry \\
& voltage\_kurtosis & 1 & Voltage kurtosis, reflecting voltage distribution tail characteristics \\
\midrule
\multirow{4}{*}{Charging Characteristics} 
& CC\_Q & 1 & Constant-current phase charging capacity (Ah), reflecting battery charging acceptance capability \\
& CC\_time & 1 & Constant-current phase charging time (s) \\
& voltage\_slope & 1 & Voltage change slope over time, related to internal resistance \\
& voltage\_entropy & 1 & Shannon entropy of voltage signal, characterizing signal complexity \\
\midrule
\multirow{8}{*}{Current Statistics} 
& current\_mean & 1 & Arithmetic mean of current sampling values within cycle \\
& current\_std & 1 & Current standard deviation, characterizing current fluctuation degree \\
& current\_skewness & 1 & Current skewness \\
& current\_kurtosis & 1 & Current kurtosis \\
& CV\_Q & 1 & Constant-voltage phase charging capacity (Ah) \\
& CV\_time & 1 & Constant-voltage phase charging time (s), usually shorter for aged batteries \\
& current\_slope & 1 & Current decay slope during constant-voltage phase \\
& current\_entropy & 1 & Shannon entropy of current signal \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Parameter Count Calculation Example}

Taking MLP as an example, the parameter count calculation formula is as follows. Baseline strategy (input dimension 16):
\begin{equation}
    \text{Params}_{\text{MLP}}^{\text{Baseline}} = (16 \times 192 + 192) + (192 \times 96 + 96) + (96 \times 48 + 48) + (48 \times 24 + 24) + (24 \times 1 + 1) = 27,649
\end{equation}

MIM strategy (input dimension 32):
\begin{equation}
    \text{Params}_{\text{MLP}}^{\text{MIM}} = (32 \times 192 + 192) + (192 \times 96 + 96) + (96 \times 48 + 48) + (48 \times 24 + 24) + (24 \times 1 + 1) = 36,865
\end{equation}
where the second term in parentheses represents bias parameters.

\subsection{Multi-Metric Comparison Results}

To verify the consistency of MIM effects across different evaluation metrics, this appendix supplements complete experimental results for RMSE and R\textsuperscript{2} metrics. As shown in Figure~\ref{fig:three_metrics}, the three metrics of MAE, RMSE, and R\textsuperscript{2} present consistent trends: MIM versions outperform Baseline versions under all missing rate conditions. This result validates the robustness of the MIM method---regardless of which commonly used regression evaluation metric is adopted, the performance improvement brought by MIM is consistent.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.58\textwidth}
        ...
        \label{fig:r2_all}
    \end{subfigure}
    \caption{Model performance comparison under three evaluation metrics: (a) MAE; (b) RMSE; (c) R\textsuperscript{2}. The three metrics present consistent trends, validating the effectiveness of the MIM method.}
