...
[TBD: Insert Table with architecture comparison results --- MIM improvement rates for MLP, LSTM, CNN with mean $\pm$ std across seeds and missing rates $\geq$ 0.3.]

[TBD: Insert Figure showing MAE curves for three architectures under Baseline and MIM, with improvement rate subplot.]
...
[TBD: Insert Figure showing MIM improvement rate as a function of parameter scale (4 points, one per Level), with error bars from 3 seeds.]
...
[TBD: Insert Figure showing MIM improvement rates for three imputation methods across 20 missing rates, with shaded confidence intervals from 3 seeds.]

[TBD: Insert Table with improvement rates at representative missing rates (MR = 0.2, 0.4, 0.6, 0.8) for each imputation method.]

A consistent gradient is observed across imputation strategies: Mean imputation yields the highest MIM improvement (27.32% at MR = 0.4), followed by Zero (15.05%) and Iterative (3.56%). This finding contradicts the initial hypothesis that simpler imputation strategies amplify MIM benefits; instead, Mean imputation's distribution-preserving property creates the most favorable condition for the model to learn effective indicator--feature interactions. The information-theoretic interpretation is that Mean imputation reduces imputation noise while retaining enough feature uncertainty for the missing indicators to provide complementary signal. Iterative imputation, despite its superior feature reconstruction, may already capture inter-feature correlations that overlap with what the missing indicators would signal, diminishing their marginal value. KNN imputation, notably, produces negative MIM improvements across most missing rates (up to $-57.7\%$ at MR = 0.65), indicating severe distributional distortion that the missing indicators cannot compensate for. This finding has important practical implications: Mean imputation represents the optimal accuracy-complexity trade-off for MIM deployment (see Section~\ref{sec:implications}).
...
[TBD: Update recommendations with actual experimental data from Layers 1--4. Expected structure: deployment guidance organized by missing rate severity and computational constraints.]
