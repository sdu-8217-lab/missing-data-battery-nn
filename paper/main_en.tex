...
\begin{abstract}
Accurate State of Health (SOH) estimation is essential for safe and reliable operation of electric vehicles and energy storage systems, yet sensor failures and data acquisition anomalies frequently cause missing monitoring data. Traditional methods treat missing values as noise, neglecting the information contained in the pattern of missingness itself. This study introduces the Missing Indicator Matrix (MIM) approach and systematically evaluates its impact on battery SOH prediction across three deep learning architectures (MLP, LSTM, 1D-CNN), four parameter scales, three imputation strategies (Zero, Mean, Iterative), and three missing mechanisms (MCAR, MAR, MNAR) using the XJTU lithium-ion battery dataset. Results demonstrate that for missing rates $\geq$0.4, MIM consistently improves prediction accuracy, reducing MAE by 60\%--70\% when combined with zero or mean imputation. The effectiveness of MIM is found to vary with imputation quality: greater improvements are observed with simpler imputation strategies (up to 70\% with Zero imputation), where missing indicators serve as stronger complementary signals. Furthermore, a multi-missing-rate joint training strategy is proposed, enabling a single MIM model to maintain robust performance across unknown missing conditions. These findings provide actionable guidance for BMS practitioners selecting model configurations based on deployment-specific missing rate characteristics.
\end{abstract}
...
\textbf{(3)} Construction of a unified evaluation benchmark covering 20 missing rates, three deep learning architectures at four parameter scales, and rigorous statistical validation through multiple random seeds, providing comprehensive insights into MIM effectiveness across diverse scenarios.
...
\toprule
\textbf{Architecture} & \textbf{Scale (Level)} & \textbf{Key Configuration} & \textbf{Baseline Params} & \textbf{MIM Params} \\
\midrule
MLP & Level 1 (Small) & hidden=[32,16,8], Dropout=0.15 & $\sim$6K & $\sim$9K \\
MLP & Level 2 (Medium) & hidden=[64,32,16], Dropout=0.15 & $\sim$12.8K & $\sim$16K \\
MLP & Level 3 (Large) & hidden=[128,64,32], Dropout=0.15 & $\sim$21.9K & $\sim$26K \\
MLP & Level 4 (XL) & hidden=[256,128,64], Dropout=0.15 & [TBD] & [TBD] \\
\midrule
LSTM & Level 1--4 & [Evaluation pending] & -- & -- \\
\midrule
CNN & Level 1--4 & [Evaluation pending] & -- & -- \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{figure*}[!htb]
    ...
    \caption{Comparison of three deep learning model architectures. (a) MLP; (b) LSTM; (c) 1D-CNN.}
...
To ensure experimental reproducibility and robustness assessment, all experiments are repeated across at least 10 independent random seeds (ranging from 0 to 9, with additional seeds 42, 123, 456, and 789 for select configurations). Each seed controls the randomness in data partitioning, missing pattern generation, and model initialization. Results are reported as mean $\pm$ standard deviation across these repeated runs, providing reliable estimates of central tendency and variability.
...
Loss function & MSE \\
Random seeds & 0--9, 42, 123, 456, 789 (10--14 runs) \\
\bottomrule
\end{tabularx}
\end{table}
...
Current limitations include: (1) Missing patterns are synthetically generated rather than collected from real-world BMS deployments, which may exhibit compound or time-correlated missing patterns not captured here; (2) Validation is conducted on the XJTU dataset with specific battery chemistries; generalization to other cell types (NMC, LFP) and datasets (CALCE, NASA) remains to be verified; (3) LSTM and CNN evaluation results are pending completion and will be incorporated in subsequent revisions; (4) The evaluation focuses on low-to-moderate quality imputation strategies (Zero, Mean, Iterative); the interaction between MIM and high-quality model-based imputation (e.g., KNN, deep generative imputation) warrants further investigation; (5) Imputation strategies are applied independently of missing mechanism type---mechanism-aware imputation may yield additional benefits. Future work should explore real-world BMS deployment data, cross-dataset validation, mechanism-aware imputation integration, and emerging model families such as Transformers and physics-informed neural networks.
