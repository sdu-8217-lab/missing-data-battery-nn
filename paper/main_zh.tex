...
然而，一个长期被忽视但在实际工程中普遍存在的问题是：电池管理系统（BMS）采集的传感器数据往往存在不同程度的缺失。这种缺失可能源于多种因素：传感器硬件故障或老化导致的测量失效、通信链路的不稳定造成的数据丢包、采样策略的优化调整引起的部分通道间歇性关闭，以及极端工况下某些保护机制触发导致的数据记录中断等。在大型储能电站或规模化电动汽车应用中，由于电池数量众多、监测点密集，数据缺失问题尤为突出。当输入特征存在缺失时，传统的数据驱动模型往往面临输入维度不匹配、特征分布偏移、预测置信度下降等问题，严重影响预测性能甚至导致模型失效。如图\ref{fig:problem_scenario}所示，实际BMS监测链路中的多源故障会导致循环级特征矩阵中出现随机缺失，如何在存在缺失的条件下保持预测性能是本文关注的核心问题。
...
缺失率（Missing Rate, MR）定义为特征矩阵中被置为缺失的元素数量占总元素数量的比例：
\begin{equation}
    \text{MR} = \frac{|\{(i,j): x_{ij} \text{ 被置为缺失}\}|}{n \times d}
    \label{eq:mr}
\end{equation}
其中，$n$为样本数量，$d$为特征维度（本研究中$d=16$），$x_{ij}$表示第$i$个样本的第$j$个特征值。
...
\end{tabular}
\\[3pt]
\footnotesize 注：*** 表示$p<$0.001，Wilcoxon符号秩检验，双尾检验，$K=$100个随机种子。
\end{table}
...
\begin{figure}[htbp]
    ...
    \caption{基于缺失率的模型选择决策建议。MR$\leq$0.3时基线模型已能满足需求；0.3$<$MR$\leq$0.6时推荐MLP-MIM或GRU-MIM（改进率75\%--86\%）；MR$\geq$0.7时强烈推荐1D-CNN-MIM（改进率$>$90\%）。}
    \label{fig:model_selection}
\end{figure}

图\ref{fig:performance_params}展示了各模型在MR=0.5时的性能-参数量权衡分析。1D-CNN-MIM位于帕累托前沿，以仅约21,300的参数实现了最低的MAE（0.014），体现了最佳的性能-参数量效率。这一特性使其特别适合在资源受限的嵌入式BMS中部署。
