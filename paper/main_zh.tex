...
本研究在Python 3.10与PyTorch 2.5.1环境下实现所有深度学习模型训练与评估，硬件平台为单块NVIDIA RTX 3090 GPU（24 GB显存）。为了确保实验结果的统计可靠性，我们采用100个不同的随机种子（42–141）对数据划分、缺失模拟和模型训练全流程进行重复，最终报告多次重复实验的均值和标准差。
