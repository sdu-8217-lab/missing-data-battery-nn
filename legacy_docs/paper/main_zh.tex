...
% 标题信息
\title{\textbf{基于缺失指示器的电池健康状态预测方法对比研究：\\跨数据集验证与电池感知缺失模式}}
...
本文在五种公开锂离子电池数据集（XJTU、HUST、MIT、TJU、NASA）上，系统评估缺失指示器方法（Missing Indicator Mechanism, MIM）在多种深度学习架构和不同缺失场景下对SOH预测性能与鲁棒性的影响。在数据预处理阶段，各数据集均已对齐为16个循环级统计特征，并以相对容量构建SOH标签；在缺失模拟阶段，除独立随机缺失外，还引入块缺失、通道缺失和状态依赖缺失三种更符合电池管理系统（BMS）实际的缺失模式，覆盖九档缺失率（Missing Rate, MR）$\{0.1, 0.2, \ldots, 0.9\}$；在训练策略上，除传统的“完整数据训练+测试时插补”基线外，特别增加了Reviewer建议的“多缺失率混合训练但不使用指示器”基线，以严格隔离MIM中“多缺失率训练”与“缺失指示器”各自的贡献。在特征增强阶段，为每个特征构造二值缺失指示器，将“零插补值与指示器拼接”作为MIM输入。我们选取MLP、LSTM、GRU与1D-CNN四种代表性深度学习模型，在统一PyTorch框架下训练和评估。

实验结果表明，在跨数据集与跨缺失模式的综合评测下，MIM方法相对于“仅插补”基线和“多缺失率训练无指示器”基线均展现出一致的性能优势，尤其在MR$\geq 0.4$时改进显著。消融分析显示，多缺失率训练本身可带来一定收益，但缺失指示器的引入是性能提升的主要来源；在块缺失、通道缺失和状态依赖缺失等BMS典型缺失模式下，MIM同样保持鲁棒。本研究通过多数据集验证与电池感知缺失模式，为MIM在电池SOH预测中的应用提供了更坚实的实证基础，并为实际BMS缺失数据处理提供了可操作的工程参考。

\textbf{关键词：}锂离子电池；健康状态（SOH）预测；缺失数据；缺失指示器（MIM）；跨数据集验证；电池感知缺失模式；深度学习
\end{abstract}
...
在此背景下，本文尝试在电池SOH预测任务中引入并系统评估一种更为简单且易于工程实现的缺失数据处理策略——缺失指示器机制（Missing Indicator Mechanism, MIM）\cite{jones1996indicator,van2023missing}。与医疗时间序列等场景不同，电池SOH预测具有鲜明的领域特性：输入为循环级统计特征，目标为单调退化的容量比值，且缺失往往由传感器故障、通信中断或老化导致的状态相关失效所引起。因此，本文并非简单移植MIM，而是针对电池数据特点进行适配：以循环为样本单位、以16维统计特征为输入，提出在训练阶段通过多缺失率混合扩充暴露模型于多样化缺失条件，并在测试阶段显式编码缺失位置。MIM在常规零值插补的基础上，为每个可能缺失的特征构造对应的二值指示变量，并与插补后的特征共同输入预测模型，使模型能够学习"哪些值缺失"与SOH之间的联合关系。已有研究表明，在缺失模式具有信息量（informative missingness）时，MIM在医疗数据分析、推荐系统等领域能够显著提升预测性能\cite{sisk2023imputation,ehrig2025imputation}。本文将在多个公开电池数据集和多种符合BMS实际的缺失模式下，通过控制良好的实验验证MIM方法在电池SOH预测任务中的有效性与泛化能力。
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{缺乏跨数据集的泛化验证}\\
    多数现有工作仅在单一数据集（如XJTU）上验证MIM效果，难以确认方法在不同化学体系、工况和采样策略下的泛化能力。电池老化数据受化学体系、温度、倍率等因素影响显著，跨数据集验证是评估方法工程适用性的必要环节。
    
    \item \textbf{缺乏隔离MIM贡献的严格基线}\\
    MIM方法通常与"多缺失率混合训练"策略结合使用，但现有研究较少对比"多缺失率训练但不使用指示器"这一关键基线。若仅与"完整数据训练+测试时插补"基线比较，则无法区分性能提升源于缺失指示器本身，还是源于训练阶段对多缺失率的暴露。
    
    \item \textbf{缺失模拟过于理想化}\\
    现有工作多采用独立伯努利随机缺失模拟，而真实BMS中的缺失通常呈现时间连续性（块缺失）、传感器通道级失效（通道缺失）或与电池老化状态相关（状态依赖缺失）。理想化的缺失模拟可能导致实验结论与实际工程场景脱节。
    
    \item \textbf{缺乏跨架构与跨缺失模式的系统比较}\\
...
针对上述研究现状与不足，本文以XJTU、HUST、MIT、TJU四个公开锂离子电池数据集为实验平台，以缺失指示器机制（MIM）为核心，构建了一套面向不同缺失率、多种缺失模式与多种神经网络架构的电池SOH预测实验框架，系统评估MIM在真实化缺失条件下的预测性能、鲁棒性与泛化能力。在统一的数据预处理、缺失模拟和训练配置下，本文试图回答以下四个核心研究问题（Research Questions, RQ）：

\begin{itemize}[leftmargin=2em]
    \item \textbf{RQ1（跨数据集泛化）：} 在五个公开数据集上，MIM相对于传统插补基线和多缺失率无指示器基线是否均能取得一致且显著的性能提升？不同数据集上的收益是否存在差异？
    
    \item \textbf{RQ2（MIM贡献分解）：} 在统一的多缺失率训练框架下，性能提升主要来源于"训练阶段暴露于多缺失率"，还是来源于"输入中显式的缺失指示器"？多缺失率无指示器基线能否达到或接近MIM的性能？
    
    \item \textbf{RQ3（电池感知缺失模式）：} 在块缺失、通道缺失和状态依赖缺失等更符合BMS实际的缺失模式下，MIM是否仍能保持相对于基线的优势？不同缺失模式对各类模型架构的影响是否存在差异？
    
    \item \textbf{RQ4（架构差异与高缺失鲁棒性）：} 对于MLP、LSTM、GRU、1D-CNN等典型神经网络架构，哪些"模型架构 + MIM"组合在极端缺失率（MR=0.9）下仍能维持可接受的预测精度与稳定性？
\end{itemize}

围绕上述研究问题，本文的主要贡献可概括为以下四个方面：

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{方法论与框架构建：}\\
    提出并实现了一套面向缺失数据场景的电池SOH预测实验流程，将缺失指示器机制系统集成到MLP、LSTM、GRU和1D-CNN四类代表性深度学习架构中。实验框架支持跨数据集加载（XJTU、HUST、MIT、TJU、NASA）、多种缺失模式（独立随机、块缺失、通道缺失、状态依赖缺失）和三类可对比训练策略（完整数据训练+测试插补基线、多缺失率训练无指示器基线、多缺失率训练+MIM），为后续相关研究提供了统一且可复现的实验基准。
    
    \item \textbf{严格基线与贡献分解：}\\
    针对Reviewer指出的基线不足问题，本文 explicitly 引入"多缺失率训练但不使用缺失指示器"基线，以隔离MIM方法中"多缺失率数据扩充"与"缺失指示器"各自的贡献，使关于MIM收益的结论更具说服力和透明度。
    
    \item \textbf{系统实证与规律揭示：}\\
    在统一的数据预处理、缺失模拟和训练设置下，本文通过跨数据集、多缺失率（MR=0.1--0.9）、多缺失模式、基于100个随机种子的控制变量大规模对比实验，系统量化了MIM在不同数据集、不同缺失模式、不同缺失率区域的性能收益，分析了不同网络架构结合MIM时的表现差异和种子稳定性，揭示了MIM收益与数据特性、缺失机制、模型结构之间的关系规律。
    
    \item \textbf{工程应用指导：}\\
    基于实验结果，本文在误差指标（如MAE、RMSE、R$^2$）、模型复杂度与缺失模式之间进行综合权衡，总结出面向不同缺失率区间、不同缺失模式和计算资源约束的模型选择建议，给出适合在实际BMS中部署的数据驱动SOH预测方案，为工程实践中处理缺失数据和选择合适的"模型+MIM"组合提供了可操作的指导意见。
\end{enumerate}
...
为验证MIM方法在不同化学体系、工况和数据采集条件下的泛化能力，本研究采用五个公开锂离子电池数据集作为实验平台：西安交通大学的XJTU数据集\cite{wang2024open,duan2025lithium}、华中科技大学发布的HUST数据集、MIT放电数据集\cite{sequura2016comparison}、天津大学的TJU数据集，以及NASA Prognostics Center of Excellence（PCoE）电池数据集\cite{saha2007battery}。前四个数据集已通过统一的特征工程流程对齐为16维循环级统计特征（见表\ref{tab:features}）和相对容量SOH标签；NASA数据集原始记录为放电循环级标量（电压、电流、温度、容量等），本研究基于前5个循环的历史窗口构造了16维循环级特征，并将容量除以初始容量得到SOH标签。所有数据集均采用相对容量SOH标签，以保证跨数据集实验的可比性。

各数据集的特点如下：XJTU数据集涵盖2C、3C、R2.5、R3、RW、Sim\_satellite等多个充放电批次；HUST数据集覆盖不同温度/工况下的10个批次；MIT数据集按测试日期分为多个子集，记录了多组电池的老化过程；TJU数据集则按化学体系（NCA、NCM、NCM+NCA）组织，包含多种采样条件下的循环数据；NASA数据集包含34个18650锂离子电池的老化记录，覆盖不同放电策略与工况。多数据集的引入使本研究能够考察MIM在不同电池化学体系、工况复杂度和采样策略下的稳定性。在NASA数据集上，本研究过滤了容量记录异常或初始容量估计不可靠的电池，最终保留25个电池用于实验。
...
为了系统评估不同缺失率条件下的模型性能，我们设计了九档缺失率$\{0.1, 0.2, \ldots, 0.9\}$的模拟实验。在缺失机制方面，本研究除采用独立随机缺失（Bernoulli MCAR）作为基准外，还引入了三种更符合电池管理系统（BMS）实际的电池感知缺失模式：

\begin{itemize}[leftmargin=2em]
    \item \textbf{块缺失（Block Missing）}：模拟传感器临时故障或通信中断。对某一特征通道，以一定概率触发一段连续时间步的缺失，缺失块长度服从指数分布。该模式体现了BMS中由于短暂通信丢包或传感器重启导致的连续缺失。
    \item \textbf{通道缺失（Channel Missing）}：模拟某个传感器完全失效或某类特征通道长期不可用。随机选择若干特征通道，在这些通道上以较高比例置缺失。该模式反映了BMS中电压、电流等单一传感器故障导致整列特征不可靠的情形。
    \item \textbf{状态依赖缺失（State-Dependent Missing）}：模拟老化加剧后信号质量下降或保护机制触发更频繁。缺失概率随SOH降低而增加，使缺失模式本身携带关于电池老化状态的额外信息。
\end{itemize}

四种缺失模式共同构成了从理想化MCAR到BMS真实缺失的递进评测体系。缺失率（Missing Rate, MR）仍按式\ref{eq:mr}定义，即特征矩阵中缺失元素占总元素的比例。
...
基于上述定义，我们构建了三种训练/输入策略供对比实验使用：

\begin{itemize}[leftmargin=2em]
    \item \textbf{基线策略（Baseline）}：模型仅在完整数据（MR=0）上训练，测试时采用零值插补后的特征向量$\hat{X} \in \mathbb{R}^{n \times 16}$作为输入。该策略模拟传统方法中"训练时无缺失、测试时缺失后插补"的常见流程。
    \item \textbf{多缺失率无指示器策略（Multi-MR w/o Indicator）}：在训练阶段将原始完整数据按九档缺失率分别生成缺失副本并合并，形成多缺失率混合训练集；输入仍为零值插补后的16维特征向量。该策略用于Reviewer建议的严格消融，以判断性能提升是否仅来自训练阶段对多缺失率的暴露。
    \item \textbf{MIM策略}：在Multi-MR训练的基础上，将零值插补后的特征向量与缺失指示器向量拼接，形成扩展的特征表示$[\hat{X} \,|\, M] \in \mathbb{R}^{n \times 32}$，其中$M$是16维的指示器向量。模型同时接收特征的值信息和缺失信息，学习缺失模式与SOH之间的联合关系。
\end{itemize}

通过上述三种策略，本文能够分别评估"多缺失率训练"和"缺失指示器"对预测性能的贡献，从而对MIM方法进行更精细的归因分析。
...
其次，在数据集方面，本研究在XJTU、HUST、MIT、TJU与NASA五个公开数据集上验证了MIM方法，覆盖了多种化学体系与工况。未来可进一步纳入CALCE、Oxford等更多数据集，并探索跨数据集迁移学习与域自适应方法，以提升模型在未知电池上的泛化能力。
