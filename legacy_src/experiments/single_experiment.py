...
class SingleExperimentRunner:
    ...
    def run(self) -> pd.DataFrame:
        ...
        # 所有模型配置 - 4模型 × 3策略 = 12配置 (已移除XGBoost)
        # 策略: baseline(完整数据), mim(多缺失率+指示器), multi_missing_rate(多缺失率无指示器)
        model_configs = [
            ModelConfig(model_type='mlp', use_mim=False, name='MLP', seq_len=5, strategy='baseline'),
            ModelConfig(model_type='mlp', use_mim=True, name='MLP-MIM', seq_len=5, strategy='mim'),
            ModelConfig(model_type='mlp', use_mim=False, name='MLP-MultiMR', seq_len=5, strategy='multi_missing_rate'),
            ModelConfig(model_type='lstm', use_mim=False, name='LSTM', seq_len=5, strategy='baseline'),
            ModelConfig(model_type='lstm', use_mim=True, name='LSTM-MIM', seq_len=5, strategy='mim'),
            ModelConfig(model_type='lstm', use_mim=False, name='LSTM-MultiMR', seq_len=5, strategy='multi_missing_rate'),
            ModelConfig(model_type='gru', use_mim=False, name='GRU', seq_len=5, strategy='baseline'),
            ModelConfig(model_type='gru', use_mim=True, name='GRU-MIM', seq_len=5, strategy='mim'),
            ModelConfig(model_type='gru', use_mim=False, name='GRU-MultiMR', seq_len=5, strategy='multi_missing_rate'),
            ModelConfig(model_type='cnn1d', use_mim=False, name='CNN1D', seq_len=5, strategy='baseline'),
            ModelConfig(model_type='cnn1d', use_mim=True, name='CNN1D-MIM', seq_len=5, strategy='mim'),
            ModelConfig(model_type='cnn1d', use_mim=False, name='CNN1D-MultiMR', seq_len=5, strategy='multi_missing_rate'),
        ]
    ...
    def _train_model(self, model_config: ModelConfig):
        ...
        # 训练策略区分
        if model_config.strategy in ('mim', 'multi_missing_rate'):
            # 多缺失率策略（带或不带指示器）
            train_data, val_data = self._prepare_multi_rate_data(model_config)
        else:
            # Baseline策略: 只用完整数据
    ...
    def _prepare_multi_rate_data(self, model_config: ModelConfig):
        """准备多缺失率训练数据 - 拼接多缺失率数据（带或不带指示器）"""
        from src.data.datasets import MIMDataset, SequenceMIMDataset, BatteryDataset, SequenceDataset
        ...
        if model_type in ['lstm', 'gru', 'cnn1d']:
            train_dataset = SequenceMIMDataset(
                self.data['X_train'], self.data['y_train'],
                seq_len=seq_len,
                missing_rates=self.config.mim_training_rates,
                use_mim=use_mim,
                base_seed=self.seed,
                missing_pattern=self.config.missing_pattern
            )
    ...
    def _prepare_mim_flat_data(self, use_mim: bool = True):
        """（已弃用，保留以兼容旧调用）使用 MIMDataset 准备数据"""
        from src.data.datasets import MIMDataset, BatteryDataset
        train_dataset = MIMDataset(
            self.data['X_train'], self.data['y_train'],
            missing_rates=self.config.mim_training_rates,
            use_mim=use_mim,
    ...
    def _prepare_mim_sequence_data(self, seq_len: int, use_mim: bool = True):
        """（已弃用，保留以兼容旧调用）使用 SequenceMIMDataset 准备数据"""
        from src.data.datasets import SequenceMIMDataset, SequenceDataset
        train_dataset = SequenceMIMDataset(
            self.data['X_train'], self.data['y_train'],
            seq_len=seq_len,
            missing_rates=self.config.mim_training_rates,
